  ...
  for (var player in players) {
    ...
    if (deck.length >= players.length) {
      ...
      if (players[player].getName() === $("#playerOneNameText").html()) {
        ...
        players[player].getName() === $("#playerThreeNameText").html()
      ) {
      ...
      showErrMsg(
        `Game Over! Not enough cards left in the deck to deal to ${
          players.length
        } players.`
      );
...
$(document).ready(() => {
  ...
  $("#playerOneLeave").click(() => {
    ...
    var filteredPlayers = players.filter(
      player => player.getName() === $("#playerOneNameText").html()
    );
  ...
  $("#playerTwoLeave").click(() => {
    // Discard current card
    var filteredPlayers = players.filter(
      player => player.getName() === $("#playerTwoNameText").html()
    );
    discardedCards.push(filteredPlayers[0].card);
  ...
  $("#playerThreeLeave").click(() => {
    // Discard current card
    var filteredPlayers = players.filter(
      player => player.getName() === $("#playerThreeNameText").html()
    );
    discardedCards.push(filteredPlayers[0].card);
